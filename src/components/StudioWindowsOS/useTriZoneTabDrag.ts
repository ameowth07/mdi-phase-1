...
import type { DocumentTabStripZone } from './documentTabStripZone'
import type { TabDragStyleClasses } from './useTabRowDragReorder'

const DRAG_THRESHOLD_PX = 5

export type TriZoneTabDragBindings = {
  mainRowRef: React.RefObject<HTMLDivElement | null>
  isoRowRef: React.RefObject<HTMLDivElement | null>
  dockRowRef: React.RefObject<HTMLDivElement | null>
  getTabProps: (zone: DocumentTabStripZone, index: number) => {
    'data-tab-draggable': 'true'
    onPointerDown: (e: ReactPointerEvent<HTMLElement>) => void
  }
  tabClass: (zone: DocumentTabStripZone, index: number, styles: TabDragStyleClasses) => string
  consumeClickAfterDrag: () => boolean
}

type DropTarget = { zone: DocumentTabStripZone; index: number }

export function useTriZoneTabDrag(handlers: {
  onReorderWithin: (zone: DocumentTabStripZone, from: number, to: number) => void
  onMoveBetween: (
...
}): TriZoneTabDragBindings {
  const mainRowRef = useRef<HTMLDivElement | null>(null)
  const isoRowRef = useRef<HTMLDivElement | null>(null)
  const dockRowRef = useRef<HTMLDivElement | null>(null)
  const dragFromRef = useRef<{ zone: DocumentTabStripZone; index: number } | null>(null)
  const draggingRef = useRef(false)
  const blockClickRef = useRef(false)
  const handlersRef = useRef(handlers)
  handlersRef.current = handlers

  const [dragFrom, setDragFrom] = useState<{ zone: DocumentTabStripZone; index: number } | null>(
    null,
  )
  ...
  const resolveDropTarget = useCallback(
    (clientX: number, clientY: number): DropTarget | null => {
      const dockRow =
        dockRowRef.current ??
        document.querySelector<HTMLDivElement>('[data-document-dock-tab-row="true"]')
      if (dockRow != null) {
        const stackEl = dockRow.closest<HTMLElement>('[data-document-dock-stack="true"]')
        const stackRect = stackEl?.getBoundingClientRect()
        const tabRect = dockRow.getBoundingClientRect()
        const hitLeft = stackRect?.left ?? tabRect.left
        const hitRight = stackRect?.right ?? tabRect.right
        if (
          clientY >= tabRect.top - 8 &&
          clientY <= tabRect.bottom + 8 &&
          clientX >= hitLeft &&
          clientX <= hitRight
        ) {
          const index = resolveDropIndexInRow(dockRow, clientX)
          if (index != null) return { zone: 'dock', index }
        }
      }
  ...
  const tabClass = useCallback(
    (zone: DocumentTabStripZone, index: number, dragStyles: TabDragStyleClasses) => {
  ...
  const getTabProps = useCallback(
    (zone: DocumentTabStripZone, index: number) => ({
  ...
  return { mainRowRef, isoRowRef, dockRowRef, getTabProps, tabClass, consumeClickAfterDrag }
}

export function tabDragClassesForTri(
  drag: TriZoneTabDragBindings,
...
export function isTriZoneTabDrag(drag: object): drag is TriZoneTabDragBindings {
  return 'dockRowRef' in drag && 'mainRowRef' in drag && 'isoRowRef' in drag
}
